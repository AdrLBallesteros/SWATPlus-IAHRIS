...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def generate_reports(self):
        ...
        initial_month = self.comboBox_mon.currentText()
        ...
        # ('exit /b' after each line propagates the first IAHRIS failure as the .bat return code)
        bat_content = f"""cd C:\\SWATPlus-IAHRIS\\IAHRIS4.0
if errorlevel 1 exit /b %errorlevel%
chcp 65001
:: Lineas de Carga de Datos.
IAHRIS.exe CD /t:P /np:"{scenario_nat_short}" /d:"Punto Carga Masiva" /p:"{project_name}" /dp:"SWATPlus-IAHRIS" /rn:"Natural" /ra:"nat" /fe:"{output_csv_path_nat}" /mi:{initial_month}
if errorlevel 1 exit /b %errorlevel%
...
"""
        ...
        # List the .xlsx files already present in the report folder, to identify
        # afterwards the workbook generated by this run and not touch any other one
        xlsx_files_before = set(glob.glob(os.path.join(report_folder, "*.xlsx")))

        # Launch the .bat file (capturing the IAHRIS console output to detect failures)
        process = subprocess.run(
            ...
            capture_output=True,
            text=True,
            encoding="utf-8",  # the .bat sets 'chcp 65001'
            errors="replace",
        )

        # Keep the IAHRIS execution log in the temp folder
        log_file_path = os.path.join(temp_folder, "iahris_log.txt")
        with open(log_file_path, "w", encoding="utf-8", errors="replace") as log_file:
            log_file.write(bat_content)
            log_file.write(f"\n:: Return code: {process.returncode}\n")
            log_file.write(f"\n:: STDOUT\n{process.stdout or ''}\n")
            log_file.write(f"\n:: STDERR\n{process.stderr or ''}\n")

        # Compare the report folder with its previous content: only the .xlsx files
        # that appeared during this run are considered
        xlsx_files = sorted(
            set(glob.glob(os.path.join(report_folder, "*.xlsx"))) - xlsx_files_before
        )

        # IAHRIS reports its own errors on the console prefixed with 'Error.' and still
        # exits with code 0, so its output has to be inspected as well
        iahris_errors = [
            line.strip()
            for line in (
                (process.stdout or "") + "\n" + (process.stderr or "")
        ...
        # Check the IAHRIS return code, the reported errors and that one single new report was generated
        if process.returncode != 0 or iahris_errors or len(xlsx_files) != 1:
            if process.returncode != 0:
                reason = f"IAHRIS did not complete the analysis (return code: {process.returncode})."
            elif iahris_errors:
                reason = "IAHRIS reported an error while processing the data."
            elif not xlsx_files:
                reason = "IAHRIS finished without reporting an error, but no report file was generated in the selected folder."
            else:
                reason = (
                    "Several new Excel files appeared in the selected folder during the analysis, "
                    "so the IAHRIS report cannot be identified:\n"
                    + "\n".join(os.path.basename(xlsx) for xlsx in xlsx_files)
                )

            # Show the message returned by IAHRIS (only the last lines, the output can be long)
            iahris_output = (
                "\n".join(iahris_errors)
                or (process.stderr or "").strip()
                or (process.stdout or "").strip()
            )
            if iahris_output:
                iahris_output = "\n".join(iahris_output.splitlines()[-15:])
            else:
                iahris_output = "IAHRIS did not return any message."

            QtWidgets.QMessageBox.critical(
                self,
                "IAHRIS Execution Error",
                f"{reason}\n\nIAHRIS output:\n{iahris_output}\n\n"
                f"The input files and the execution log have been kept in:\n{temp_folder}",
            )
        ...
        # Remove the temp folder (only once the execution has been confirmed)
        ...
        try:
            if xlsx_files:
                # Get the .xlsx file newly generated by this IAHRIS run
                last_generated_xlsx = xlsx_files[0]
